...
async function getValidAccessToken(portal_id_input) {
  ...
  if (!inst) throw new Error(`No installation found for portal=${portal}`);

  const accessToken = pick(inst, ['access_token']);
  const refreshToken = pick(inst, ['refresh_token']);
  const expiresAtRaw = pick(inst, ['expires_at']);
